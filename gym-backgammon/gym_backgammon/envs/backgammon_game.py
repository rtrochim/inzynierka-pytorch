...
class BackgammonGame:
    ...
    def get_bear_off_play_double(self, player, roll):
        ...
        if len(out_of_home_move) > 0:
            ...
            self.make_move(player, out_of_home_move)
            self.players_positions = self.get_players_positions()
    ...
    def get_valid_actions(self, player, roll):
        valid_plays = set()
        top_valid_plays = set()
    ...
    def make_move(self, current_player, action):
        if action:
            tmp = self.board[:]
            for move in action:
                if move:
    ...
    def get_observation(self, current_player):
        features_vector = []
